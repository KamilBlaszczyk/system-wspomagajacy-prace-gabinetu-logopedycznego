...
function backup_database( $directory, $outname , $dbhost, $dbuser, $dbpass ,$dbname ) {
  ...
  $dir  = dirname("backup/backup/"); // directory files
$name = 'backup_'.date("YmHis"); // name sql backup
print_r( backup_database( $dir, $name, $db_server,$db_username,$db_password, $db_name) ); // execute

echo '<br><a href="'.$_SERVER['DOCUMENT_ROOT'].'/admin/backup/'.$name.'.sql.gz">Kliknij tutaj</a>, aby pobrać.</a>';
  ?>